...
from tkinter import simpledialog, filedialog, messagebox, ttk, font
import os
import shutil
import sys
import json
...
class NewObject:
    def __init__(self, root, object_name, canvas, scrollregion, tree, positions, directory, object_names, back_button, x, y):
        ...
        root.protocol("WM_DELETE_WINDOW", self.delete_object)
        ...
        # Intitialize data structure for saving object information
        ...
        # Import from button
        self.import_from_button = ttk.Button(self.frame.interior, image=self.import_img, command = self.import_from, bootstyle='light')
        self.import_from_button.grid(row=1, column=0)
        Tooltip(self.import_from_button, "Import data from ...")
        ...
        self.object_dict["folder"] = value_var

        # Objectives Frame
        self.objective_frame = tk.Frame(self.frame.interior, bg = 'white')
        self.objective_frame.grid(row=3, column=0, columnspan=5)
        self.objective_label = tk.Label(self.objective_frame, text="Objectives", font=("Helvetica", 14), bg='white')
        ...
        self.add_objective_button.pack()
        ...
        self.delete_objective_button.pack()
        ...
        self.attributes_tol_button = ttk.Button(self.attribute_frame, image=self.show_img, text=" Tolerances", compound='left', command=self.attribute_tol, bootstyle='light')
        ...
        self.file_label = tk.Label(self.file_frame, text="Files", font=("Helvetica", 14), bg = 'white')
        ...
        self.reference_label = tk.Label(self.reference_frame, text = 'References', font=("Helvetica", 14), bg = 'white')
        ...
        self.subscriber_label.grid(row=0, column=0)
    ...
    def save(self):
        if self.object_dict["folder"].get() == '':
            messagebox.showinfo("No folder selected", "Please select a folder for the object.")
        else:
            # transform tkinter data types used for entry boxes to string values:
            for i in range(len(self.object_dict["objectives"])):
                self.object_dict["objectives"][i] = (self.object_dict["objectives"][i][0], self.object_dict["objectives"][i][1].get())
            for i in range(len(self.object_dict["attributes"])):
                if self.object_dict["attributes"][i][4] == []:
                    self.object_dict["attributes"][i] = [self.object_dict["attributes"][i][0], self.object_dict["attributes"][i][1].get(),
                                                        self.object_dict["attributes"][i][2].get(), self.object_dict["attributes"][i][3].get(), []]
                else:
                    tols = (self.object_dict["attributes"][i][3][0].get(), self.object_dict["attributes"][i][3][1].get())
                    self.object_dict["attributes"][i] = [self.object_dict["attributes"][i][0], self.object_dict["attributes"][i][1].get(),
                                                        self.object_dict["attributes"][i][2].get(), self.object_dict["attributes"][i][3].get(), tols]
            for i in range(len(self.object_dict["inputs"])):
                self.object_dict["inputs"][i] = [self.object_dict["inputs"][i][0], self.object_dict["inputs"][i][1].get(),
                                                self.object_dict["inputs"][i][2].get(), self.object_dict["inputs"][i][3].get()]
            for i in range(len(self.object_dict["outputs"])):
                self.object_dict["outputs"][i] = [self.object_dict["outputs"][i][0], self.object_dict["outputs"][i][1].get(),
                                                self.object_dict["outputs"][i][2].get(), self.object_dict["outputs"][i][3].get()]
            for i in range(len(self.object_dict["references"])):
                self.object_dict["references"][i] = self.object_dict["references"][i].get()
            self.object_dict["folder"] = self.object_dict["folder"].get()        
            ...
            if os.path.isfile(file_path): # file already exists, put old information in old dict and new information in new dict
                with open(file_path, "r") as f: # read content first and fill object_saving_dict
                    object_info = f.read()
            ...
            with open(file_path, "w+") as f: # store object_saving_dict in text file
                f.write(str(object_saving_dict))
            ...
            parent = self.object_dict['folder']
            self.tree.insert(parent, 'end', text=self.object_name, iid = self.object_name)

            # write object_name into referenced objects' references (references are always mutual):
            for ref in self.object_dict["references"]:
    ...
    def save_publish(self):
        ...
        for obj in self.object_dict["references"]:
            changements[obj] = object_changements(obj, os.path.join(self.directory, 'current', obj))
    ...
    def add_objective(self):
        ...
        if objective:
            ...
            self.object_dict["objectives"].append([objective, state_var])
            
    def delete_objective(self):
        if self.objective_checkbuttons:
            self.object_dict["objectives"].pop()
    ...
    def add_attribute(self):
        ...
        if attribute:
            length = len(self.object_dict["attributes"])
            ...
            if self.attribute_tol_plus.winfo_ismapped():
                ...
                self.object_dict["attributes"].append([attribute, shortcut_var, value_var, unit_var, [plus_var, minus_var]])
            else:
                self.object_dict["attributes"].append([attribute, shortcut_var, value_var, unit_var, []])

    def delete_attribute(self):
        if self.attribute_labels:
            self.object_dict["attributes"].pop()
            for i in range(4):
                self.attribute_labels.pop().destroy()
            if self.attribute_tol_labels:
                self.attribute_tol_labels.pop().destroy()
                self.attribute_tol_labels.pop().destroy()
            
            # remove Name, Symbol, Value, +, - all attributes are removed
            if self.object_dict["attributes"] == []:
    ...
    def add_input(self):
        ...
        if input_:
            length = len(self.object_dict["inputs"])
            if self.object_dict["inputs"] == []:
                self.input_name_label.grid(row=4, column=0)
                self.input_shortcut_label.grid(row=4, column=1)
                self.input_value_label.grid(row=4, column=2)
                self.input_unit_label.grid(row=4, column=3)
            self.input_labels.append(tk.Label(self.input_frame, text=input_, bg = 'white'))
            self.input_labels[-1].grid(row=length+5, column=0)

            shortcut_var = tk.StringVar(self.master)
            self.input_labels.append(tk.Entry(self.input_frame, textvariable=shortcut_var))
            self.input_labels[-1].grid(row=length+5, column=1)

            value_var = tk.StringVar(self.master) 
            self.input_labels.append(tk.Entry(self.input_frame, textvariable=value_var))
            self.input_labels[-1].grid(row=length+5, column=2)

            unit_var = tk.StringVar(self.master) 
            self.input_labels.append(tk.Entry(self.input_frame, textvariable=unit_var))
            self.input_labels[-1].grid(row=length+5, column=3)

            self.object_dict["inputs"].append([input_, shortcut_var, value_var, unit_var])

    def delete_input(self):
        if self.input_labels:
            self.object_dict["inputs"].pop()
            for i in range(4):
    ...
    def add_output(self):
        ...
        if output:
            length = len(self.object_dict["outputs"])
            if self.object_dict["outputs"] == []:
                self.output_name_label.grid(row=4, column=0)
                self.output_shortcut_label.grid(row=4, column=1)
                self.output_value_label.grid(row=4, column=2)
                self.output_unit_label.grid(row=4, column=3)
            self.output_labels.append(tk.Label(self.output_frame, text=output, bg = 'white'))
            self.output_labels[-1].grid(row=length+5, column=0)

            shortcut_var = tk.StringVar(self.master)
            self.output_labels.append(tk.Entry(self.output_frame, textvariable=shortcut_var))
            self.output_labels[-1].grid(row=length+5, column=1)

            value_var = tk.StringVar(self.output_frame) 
            self.output_labels.append(tk.Entry(self.output_frame, textvariable=value_var))
            self.output_labels[-1].grid(row=length+5, column=2)

            unit_var = tk.StringVar(self.output_frame) 
            self.output_labels.append(tk.Entry(self.output_frame, textvariable=unit_var))
            self.output_labels[-1].grid(row=length+5, column=3)

            self.object_dict["outputs"].append([output, shortcut_var, value_var, unit_var])

    def delete_output(self):
        if self.output_labels:
            self.object_dict["outputs"].pop()
            for i in range(4):
    ...
    def add_file(self):
        ...
        if file:
            length = len(self.object_dict["files"])
            ...
            self.object_dict["files"].append(file)
    ...
    def choose_file(self):
        ...
        for i, file in enumerate(file_list_without_filetypes):
            if file == self.object_dict["files"][-1]:
        ...
        if file:
            ...
            new_file_name = str(self.object_dict["files"][-1])+str(filetype)
    ...
    def delete_file(self):
        delete = messagebox.askyesno(title="Delete file and objects", message = "Are you sure you want to delete this file and the objects within?")
        if delete:
            if self.file_labels:
                # remove copied file from object's directory:
                file_list = os.listdir(os.path.join(self.directory, 'current', self.object_name)) # get list of files in object's directory
                file_list_without_filetypes = [file.rsplit(".", 1) for file in file_list] # remove endings
                for i, file in enumerate(file_list_without_filetypes):
                    if file == self.object_dict["files"][-1]:
                        os.remove(os.path.join(self.directory, 'current', self.object_name, file_list[i]))
                # remove file from object dict, remove buttons and label:
                self.object_dict["files"].pop()
    ...
    def add_reference(self):
        ...
        current_refs = [ref.get() for ref in self.object_dict["references"]] # current references: get strings from tk.strings
        ...
        if options:
            ...
            self.object_dict["references"].append(value_var)
        else:
    ...
    def delete_reference(self):
        ...
        self.object_dict["references"][-1].pop()
    ...
    def attribute_tol(self):
        ...
        if not self.attribute_tol_plus.winfo_ismapped():
            ...
            self.attribute_tol_plus.grid(row=4, column=4)
            self.attribute_tol_minus.grid(row=4, column=5)
            # create tolerance entry boxes for every already existing attribute:
            if self.object_dict["attributes"]:
                for i in range(len(self.object_dict["attributes"])):
                    plus_var = tk.StringVar(self.attribute_frame)
                    self.attribute_tol_labels.append(tk.Entry(self.attribute_frame, textvariable=plus_var))
                    ...
                    self.object_dict["attributes"][i][3] = [plus_var, minus_var]
        # remove tolerances from every attribute:
            ...
            self.attribute_tol_minus.grid_forget()
            if self.object_dict["attributes"]:
                l = len(self.object_dict["attributes"])
                for i in range(l):
                    self.attribute_tol_labels.pop().destroy()
                    self.attribute_tol_labels.pop().destroy()
                    self.object_dict["attributes"][i][3] = []
    ...
    def import_from(self):
        ...
        if dialog1.result == 'CSV':
            options2 = ['Everything', 'Only defined attributes']
            ...
            if import_directory:
                import_data = data_from_csv(import_directory, dialog2.result, self.object_dict)
            ...
            import_directory = filedialog.askopenfilename(filetypes=files)
            if import_directory:
                import_data = data_from_pdf(import_directory, dialog2.result, self.object_dict)
            ...
            import_directory = filedialog.askopenfilename(filetypes=files)
            if import_directory:
                import_data = data_from_txt(import_directory, dialog2.result, self.object_dict)
        elif dialog1.result == 'PNG/JPG':
            dialog2_res = 'Only defined attributes'
            files = [('PNG', '.png'), ('JPG', '.jpg')]
            import_directory = filedialog.askopenfilename(filetypes=files)
            if import_directory:
                import_data = data_from_image(import_directory, self.object_dict)

        if import_data:
            self.insert_import_data(dialog2_res, import_data)
    
    def insert_import_data(self, dialog2, import_data):        
        if dialog2 == 'Only defined attributes':
            for (ilabel, state) in import_data["objectives"]:
                for i, (label,_) in enumerate(self.object_dict["objectives"]):
                    if ilabel == label:
                        if state == 1:
            ...
            for (ilabel, shortcut, value, unit, tol) in import_data["attributes"]:
                for i, (label,_,_,_,_) in enumerate(self.object_dict["attributes"]):
                    if ilabel == label:
                        ...
                        if tol:
                            ...
                            if not self.attribute_tol_plus.winfo_ismapped():
                                ...
                                self.attribute_tol_plus.grid(row=4, column=4)
                                self.attribute_tol_minus.grid(row=4, column=5)
                                for j in range(len(self.object_dict["attributes"])):
                                    plus_var = tk.StringVar(self.attribute_frame)
                                    self.attribute_tol_labels.append(tk.Entry(self.attribute_frame, textvariable=plus_var))
                                    ...
                                    self.object_dict["attributes"][j][4] = [plus_var, minus_var]
            ...
            for (ilabel, shortcut, value, unit) in import_data["inputs"]:
                for i, (label,_,_,_) in enumerate(self.object_dict["inputs"]):
                    if ilabel == label:
            ...
            for (ilabel, shortcut, value, unit) in import_data["outputs"]:
                for i, (label,_,_,_) in enumerate(self.object_dict["outputs"]):
                    if ilabel == label:
            ...
            for (ilabel, value) in import_data["files"]:
                for i, (label,_) in enumerate(self.object_dict["files"]):
                    if ilabel == label:
            ...
            for ob in import_data["objectives"]:
                name, state = ob
                l = len(self.object_dict["objectives"])
                state_var = tk.IntVar(self.objective_frame)
                self.objective_checkbuttons.append(tk.Checkbutton(self.objective_frame, text = name, variable=state_var))
                self.objective_checkbuttons[-1].pack()
                if state == 1:
                    self.objective_checkbuttons[-1].select()
                self.object_dict["objectives"].append([name, state_var])

            for at in import_data["attributes"]:
                if not self.object_dict["attributes"]:
                    self.attribute_name_label.grid(row=4, column=0)
                    self.attribute_shortcut_label.grid(row=4, column=1)
                    self.attribute_value_label.grid(row=4, column=2)
                    self.attribute_unit_label.grid(row=4, column=3)
                ...
                self.object_dict["attributes"].append([name, shortcut_var, value_var, unit_var, []])
                ...
                if self.attribute_tol_plus.winfo_ismapped(): #self.attributes_tol_button.cget('text') == 'Remove attribute tolerances':
                    ...
                    self.attribute_tol_labels.append(tk.Entry(self.attribute_frame, textvariable=minus_var))
                    self.attribute_tol_labels[-1].grid(row=length+5, column=5)
                    self.object_dict["attributes"][-1][4] = [plus_var, minus_var] # append tolerance variables to last entry of object dict
                    if tol:
                    ...
                    if tol: # if imported data has tolerances, add tolerance entry boxes to every already existing attribute
                        ...
                        for i in range(len(self.object_dict["attributes"])):
                            ...
                            self.attribute_tol_labels.append(tk.Entry(self.attribute_frame, textvariable=minus_var))
                            self.attribute_tol_labels[-1].grid(row=i+5, column=5)
                            self.object_dict["attributes"][i][4] = [plus_var, minus_var]
                        self.attribute_tol_labels[-2].insert(0, tol[0])
                        self.attribute_tol_labels[-1].insert(0, tol[1])

            for inp in import_data["inputs"]:
                if not self.object_dict["inputs"]:
                ...
                self.object_dict["inputs"].append([name, shortcut_var, value_var, unit_var])

            for ou in import_data["outputs"]:
                if not self.object_dict["outputs"]:
                ...
                self.object_dict["outputs"].append([name, shortcut_var, value_var, unit_var])
            ...
            for fi in import_data["files"]:
                ...
                self.file_labels[-1].grid(row=length+3, column=1)
                self.object_dict["files"].append(name)

            for value in import_data["references"]:
                if value != self.object_name:
                    length = len(self.object_dict["references"])
                    options = list(self.object_names.keys())
